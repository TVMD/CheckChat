...
public class StartView extends RatingBar {

    private int colorOutlineFrom = Color.rgb(0x11, 0x11, 0x11);
    private int colorOutlineTo = Color.rgb(0x61, 0x61, 0x61);
    private int colorFillFrom = Color.rgb(0xFF, 0x98, 0x00);
    private int colorFillTo = Color.TRANSPARENT;
    ...
    private float value;
    private int transparentBackground = Color.TRANSPARENT;
    private int heightView = 20;
    ...
    protected synchronized void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        int desiredWidth = (int) (heightView * getNumStars());
    ...
    protected void onDraw(Canvas canvas) {
        ...
        BitmapShader shaderFill = updateShader(colorFillFrom, colorFillTo);
        paintInside.setShader(new LinearGradient(getWidth() / 2, 0, getWidth() / 2, getHeight(), colorFillFrom, colorFillTo, Shader.TileMode.CLAMP));
        ...
        for (int i = 0; i < getNumStars(); ++i) {
            ...
            paintOutline.setShader(new LinearGradient(getWidth() / 2, 0, getWidth() / 2, getHeight(), colorOutlineFrom, colorOutlineTo, Shader.TileMode.CLAMP));
            ...
            paintW.setColor(transparentBackground);


            if (getValue() == 0.5f)
                canvas.drawRect(getWidth() / 2, 0, getWidth(), getHeight(), paintW);
            if (getValue() == 0.0f) {
                canvas.drawRect(0, 0, getWidth(), getHeight(), paintW);
            }
    ...
    private void getXmlAttrs(Context context, AttributeSet attrs) {
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.StartView, 0, 0);
        try {
            colorOutlineFrom = a.getInteger(R.styleable.StartView_colorOutlineFrom, Color.rgb(0x11, 0x11, 0x11));
            colorOutlineTo = a.getInteger(R.styleable.StartView_colorOutlineTo, Color.rgb(0x61, 0x61, 0x61));
            colorFillFrom = a.getInteger(R.styleable.StartView_colorFillFrom, Color.rgb(0xFF, 0x98, 0x00));
            colorFillTo = a.getInteger(R.styleable.StartView_colorFillTo, Color.TRANSPARENT);
            polygonVertices = a.getInteger(R.styleable.StartView_polygonVertices, 5);
            polygonRotation = a.getInteger(R.styleable.StartView_polygonRotation, 0);
            strokeWidth = (int) a.getDimension(R.styleable.StartView_strokeWidth, -1);
        } finally {
            a.recycle();
        }
    }

    public void setColorOutlineFrom(int colorOutlineFrom) {
        this.colorOutlineFrom = colorOutlineFrom;
    }

    public void setColorOutlineTo(int colorOutlineTo) {
        this.colorOutlineTo = colorOutlineTo;
    }


    public void setColorFillFrom(int colorFillFrom) {
        this.colorFillFrom = colorFillFrom;
    }

    public void setColorFillTo(int colorFillTo) {
        this.colorFillTo = colorFillTo;
    }
